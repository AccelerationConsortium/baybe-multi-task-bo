...
if __name__ == "__main__":
    ...
    # LOAD DATA-----------------------------------------------------------------------------------
    dfMP = pd.read_csv(os.path.join(strWD, "data", "mp_bulkModulus_cleaned.csv"), index_col=0)
    logging.info("Loaded bulk modulus data from csv file")

    dfExp = pd.read_csv(os.path.join(strWD, "data", "exp_hardness_cleaned.csv"), index_col=0)
    ...
    dfMP = ElementFraction().featurize_dataframe(dfMP, col_id="composition", ignore_errors=True, return_errors=True)

    # -----GET COMPOSITIONAL DATA FOR EXPERIMENTAL HARDNESS DATA-----
    # convert the strComposition to a composition object
    dfExp = StrToComposition().featurize_dataframe(dfExp, "strComposition")
    # get the element fractions
    dfExp = ElementFraction().featurize_dataframe(dfExp, col_id="composition", ignore_errors=True, return_errors=True)
    ...
    logging.info("Exported experimental hardness data with element fractions to csv file")
