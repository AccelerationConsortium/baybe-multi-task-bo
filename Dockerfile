...
# and `pip install baybe-multi-task-bo` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "baybe-multi-task-bo", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n baybe-multi-task-bo -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "baybe-multi-task-bo", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "baybe-multi-task-bo", "YOUR_CONSOLE_SCRIPT"]
